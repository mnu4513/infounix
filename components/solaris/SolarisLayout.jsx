// components/solaris/SolarisLayout.jsx
...
const SolarisLayout = ({ children, activeSlug }) => {
  ...
  // For static pages like /course/solaris/introduction-to-solaris
  let pathSlug = null;
  if (typeof router.asPath === "string") {
    if (router.asPath.startsWith("/course/solaris/")) {
      pathSlug = router.asPath.replace("/course/solaris/", "").split("?")[0];
  ...
  const isOverviewRoute =
    router.pathname === "/course/solaris" || (!currentSlug && pathSlug === null);
      ...
      <div className="relative mx-auto flex max-w-6xl gap-4 px-4 pb-10 pt-5 md:px-6 lg:px-8">
        ...
        >
          ...
          <nav className="space-y-1 text-xs pb-4">
            {solarisLessons.map((lesson) => {
              const href = lesson.slug
                ? `/course/solaris/${lesson.slug}`
                : "/course/solaris";
        ...
        <section className="ml-0 flex-1 md:ml-2">
          ...
          <div className="mb-4 flex items-center justify-between md:hidden">
            ...
            <p className="text-xs text-slate-400">
              Solaris ·{" "}
              {currentSlug
                ? solarisLessons.find((l) => l.slug === currentSlug)?.title ??
                  "Lesson"
                : "Overview"}
            </p>
          </div>
